package liuyuyang.net.vo.file;

import io.swagger.v3.oas.annotations.media.Schema;
import lombok.Data;

import java.util.ArrayList;
import java.util.List;

@Data
@Schema(name = "FileTreeNodeVO", description = "文件树中的目录节点")
public class FileTreeNodeVO {
    @Schema(description = "节点类型：dir")
    private String type;

    @Schema(description = "目录名")
    private String name;

    @Schema(description = "目录路径前缀")
    private String path;

    @Schema(description = "子目录")
    private List<FileTreeNodeVO> children = new ArrayList<>();

    @Schema(description = "当前目录下的文件")
    private List<FileTreeFileVO> files = new ArrayList<>();

    @Schema(description = "目录下文件数量（递归统计真实文件）")
    private Integer fileCount;

    @Schema(description = "目录下文件总大小（字节）")
    private Long totalSize;
}
